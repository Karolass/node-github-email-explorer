...
let apiReq = async(function* (URL, method) {
  try {
    let response = yield request({
      ...
      method: method,
      headers: {
        "User-Agent": "node-github-email-explorer",
      }
    });
...
module.exports = {
  ...
  stargazersEmails: function* (repo_user, repo_name, github_api_auth) {
    ...
    yield this.emailProcess(userIds, github_api_auth, "stargazers");
  },
  forksEmails: function* (repo_user, repo_name, github_api_auth) {
    ...
    yield this.emailProcess(userIds, github_api_auth, "forks");
  },
  emailProcess: function* (user_ids, github_api_auth, tableHead) {
    ...
    let table = new Table({ head: [`${tableHead} count`, 'with email']});
